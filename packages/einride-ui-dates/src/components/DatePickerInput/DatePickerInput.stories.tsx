...
const ControlledTemplate = (args: ComponentProps<typeof DatePickerInput>): React.JSX.Element => {
  const [value, setValue] = useState<Date | null>(null)
  return <DatePickerInput {...args} value={value} onChange={setValue} />
}
    ...
    <SnapshotWrapper alignItems="stretch">
      {[Basic, WithoutLabel, DefaultValue, USFormat, Message, SuccessMessage, ErrorMessage].map(
        (Story, index) => (
          ...
          <DatePickerInput key={index} {...Story.args} />
        ),
      )}
    </SnapshotWrapper>
  ),
