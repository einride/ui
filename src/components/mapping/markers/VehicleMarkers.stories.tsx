import { ComponentMeta, ComponentStoryObj } from "@storybook/react"
import { SnapshotWrapper } from "../../../lib/storybook/SnapshotWrapper"
import { Group } from "../../layout/Group/Group"
import { Stack } from "../../layout/Stack/Stack"
import { Text } from "../../typography/Text/Text"
import { VehicleState, VehicleMarkerName } from "../types"
import { useVehicleMarker } from "./useMapMarker"

const VEHICLE_NAMES: VehicleMarkerName[] = ["cet", "aet"]
const VEHICLE_STATES: VehicleState[] = ["default", "warning"]

export default {
  title: "Mapping/Markers/Vehicle",
} satisfies ComponentMeta<typeof Template>

type Story = ComponentStoryObj<typeof Template>

const Marker = ({ name, state }: { name: VehicleMarkerName; state: VehicleState }): JSX.Element => {
  const { getMarker } = useVehicleMarker()
  const icon = getMarker(name, state)
  return (
    <Group background="tertiary" style={{ alignItems: "center" }} padding="md">
      <img src={icon.url} alt={name} />
      <Text variant="bodySm" font="mono">
        {[name, state || "default"].join(", ")}
      </Text>
    </Group>
  )
}

const Template = (): JSX.Element => {
  return (
    <Stack gap="none">
      {VEHICLE_NAMES.map((name) => {
        return VEHICLE_STATES.map((state) => {
          return <Marker key={`${name}_${state}`} name={name} state={state} />
        })
      })}
    </Stack>
  )
}

export const All = {
  render: () => <Template />,
} satisfies Story

export const One = {
  args: {
    name: "aet",
    state: "default",
  },
  argTypes: {
    name: {
      control: "radio",
      options: VEHICLE_NAMES,
    },
    state: {
      control: "radio",
      options: VEHICLE_STATES,
    },
  },
  render: (args) => <Marker {...args} />,
} satisfies ComponentStoryObj<typeof Marker>

export const Snapshot = {
  render: () => (
    <SnapshotWrapper alignItems="stretch">
      {[All].map((_, index) => (
        // eslint-disable-next-line react/no-array-index-key
        <Template key={index} />
      ))}
    </SnapshotWrapper>
  ),
  parameters: { chromatic: { disableSnapshot: false } },
} satisfies Story
